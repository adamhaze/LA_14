...
####Compared to Lab 5:
Our new answer shows that the Corn Belt states are producing more milk than the mountain states which are usually producing less than average yearly. This gives us more insight compared to our old answer because we were limited by only analyzing the statistics by state and not by region. However, the results from both labs do agree. Lab 14 just used more advanced techniques to generalize the results into regions. 
...
```{r echo = FALSE, results = 'asis'}
library(knitr)
kable(realDifference, caption = "The percentiles of difference in means in 1970 and 2017")
```
```{r echo = FALSE, results = 'asis'}
library(knitr)
kable(percentile_data, caption = "The percentiles of the distribution of Difference in Means from Random Groupings of years 1970 and 2017")
```
